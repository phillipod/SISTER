...
class BuildClassifier:
    """
    Determines the likely build type of a Star Trek Online character screenshot using spatial and textual label analysis.
    ...
    def classify(self, label_positions):
        """
    ...
    def _score_with_rules(self, labels, rule_set, build_type):
    ...
    def _check_vertical_stack(self, labels, required_labels, align="left"):
        """
    ...
    def _check_horizontal_alignment(self, labels, required_labels):
        """
    ...
    def _is_left_of(self, labels, left_label, right_label):
        """
    ...
    def _labels_vertically_between(self, labels, label1, label2, group):
        """
    ...
    def _score_pc_ship_build(self, labels):
        """
        Compute the score for a PC Ship Build based on known label positions.
        ...
        """
        required = ["Fore Weapon", "Deflector", "Impulse", "Warp", "Shield", "Aft Weapon", "Engineering Console", "Science Console", "Tactical Console"]
        score = sum(10 for label in required if label in labels)
        logger.debug(f"PC Ship Build: Presence score {score}")
        if self._check_vertical_stack(labels, required, align="right"):
            score += 50
            logger.debug("PC Ship Build: Vertical stack matched")
        if self._labels_vertically_between(labels, "Fore Weapon", "Aft Weapon", required):
            score += 70
            logger.debug("PC Ship Build: Labels between matched")

        return score

    def _score_pc_ground_build(self, labels):
        """
        Compute the score for a PC Ground Build based on known label positions.
        ...
        Returns:
            int: Score representing the confidence of this build type.
        """
        required = ["Kit", "Body", "Shield", "Weapon"]
        score = 0
        if "Kit Module" in labels:
            score += 20
            logger.debug("PC Ground Build: Kit Module present")
        presence_score = sum(10 for label in required if label in labels)
        score += presence_score
        logger.debug(f"PC Ground Build: Presence score {presence_score}")
        if self._check_vertical_stack(labels, required, align="left"):
            score += 50
            logger.debug("PC Ground Build: Vertical stack matched")
        return score

    def _score_console_ship_build(self, labels):
        """
        Compute the score for a Console Ship Build based on known label positions.
        ...
        Returns:
            int: Score representing the confidence of this build type.
        """
        left = ["Fore Weapon", "Aft Weapon", "Devices"]
        right = ["Engineering Console", "Science Console", "Tactical Console"]
        score = 0
        presence_score = sum(10 for label in left + right if label in labels)
        score += presence_score
        logger.debug(f"Console Ship Build: Presence score {presence_score}")
        if self._check_vertical_stack(labels, left, align="left"):
            score += 50
            logger.debug("Console Ship Build: Left vertical stack matched")
        if self._check_vertical_stack(labels, right, align="left"):
            score += 50
            logger.debug("Console Ship Build: Right vertical stack matched")
        if self._is_left_of(labels, "Fore Weapon", "Engineering Console"):
            score += 30
            logger.debug("Console Ship Build: Fore Weapon is left of Engineering Console")
        if self._labels_vertically_between(labels, "Fore Weapon", "Aft Weapon", left + right):
            score -= 70
            logger.debug("Console Ship Build: Labels between matched, penalizing score")

        return score

    def _score_console_ground_build(self, labels):
        """
        Compute the score for a Console Ground Build based on known label positions.
        ...
        Returns:
            int: Score representing the confidence of this build type.
        """
        if "Kit Frame" not in labels:
            logger.debug("Console Ground Build: Missing required label 'Kit Frame'")
            return 0

        required = ["Weapon", "Body", "Kit"]
        score = 0
        presence_score = sum(10 for label in required if label in labels)
        score += presence_score
        logger.debug(f"Console Ground Build: Presence score {presence_score}")
        if self._check_vertical_stack(labels, required, align="left"):
            score += 50
            logger.debug("Console Ground Build: Vertical stack matched")
        if self._is_left_of(labels, "Weapon", "Devices"):
            score += 30
            logger.debug("Console Ground Build: Weapon is left of Devices")
        if self._check_horizontal_alignment(labels, ["Body", "Shield", "Kit Frame"]):
            score += 30
            logger.debug("Console Ground Build: Horizontal alignment matched")
        return score
