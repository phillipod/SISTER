...
1.  **Create a ForwardEmail Account:**
    *   Sign up at [forwardemail.net](https://forwardemail.net/).

2.  **Verify Your Domain:**
    *   Follow ForwardEmail's instructions to add and verify your sending domain. This is crucial for email deliverability.

3.  **Get Your API Key:**
    *   Navigate to your account settings in ForwardEmail and generate an API key. Note this key securely.

4.  **Configure Environment Variables:**
    Add the following to your `.env` file or server environment:
    ```bash
    FORWARD_EMAIL_API_KEY='your_forwardemail_api_key_here'
    FORWARD_EMAIL_FROM_EMAIL='your_verified_sender_email@yourdomain.com'
    FORWARD_EMAIL_REPLY_TO='your_reply_to_address@yourdomain.com' # Optional, defaults to FROM_EMAIL
    ```

    *   Replace placeholders with your actual ForwardEmail API key and verified email addresses.

5.  **Set Up Inbound Email (Optional):**
    *   To process user replies for license acceptance, you need to set up an inbound webhook in ForwardEmail.
    *   In ForwardEmail, go to "Webhooks" and create a new webhook.
    *   Set the webhook URL to `https://yourdomain.com/api/email-webhook`
    *   Enable the "Email Received" event
    *   ForwardEmail will sign webhook requests with a signature in the `X-ForwardEmail-Webhook-Signature` header

    **Note:** The application's `app.py` is set up to handle these environment variables. The `FORWARD_EMAIL_FROM_EMAIL` must be an email address associated with a verified domain in your ForwardEmail account.
